...
class CaptureThompson(SensingAlgorithm):

	def __init__(self, process, estimator,w,initial_data = None, dt = 10.,topk = 1):

		super().__init__(process,w,dt=dt, topk = topk)
...
if __name__ == "__main__":
	...
	estimator = PositiveRateEstimator(process, hierarchical_structure, kernel_object=k, B=B + b, b = b, m=m, jitter=10e-3,
									  estimator='likelihood', feedback="histogram")
	...
	#w = lambda s: s.volume()
	w = lambda s: s.volume() + 10.

	Bandit = CaptureThompson(process,estimator,w, initial_data = data, dt = dt)
	T = 50
	plt.ion()
	...
	for t in range(T):
		...
		for index,action in enumerate(actions):
			...
			if index == 0:
				plt.plot(xx,xx*0+(scores[index]*w(action))/action.volume()/dt,'g',lw = 2, label = 'Thompson')
			else:
